# new better build system with clang
import os
import argparse
import platform
import shutil
...
for i in range(len(files_to_build)):
    exit_code = os.system(f"clang -target i686-unknown-elf -c {os.getcwd() + '/src/' + files_to_build[i]} -o {'build/' + files_to_build_no_extensions[i] + '.o'} -std=gnu99 -ffreestanding -O2 -Wall -Wextra -I{os.getcwd()}/include")
...
exit_code = os.system(f"clang -target i686-elf -T linker.ld -o build/Azbest_OS.bin -ffreestanding -O2 -nostdlib {link} -lgcc")
